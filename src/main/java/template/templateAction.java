...
public class templateAction extends BaseExtensionRestHandler {
	...
	public List<NamedRoute> routes() {
		return List.of(
				new NamedRoute.Builder().method(Method.PUT)
						.path("/sample")
						.uniqueName("template_extension:sample/create")
						.handler(createHandler)
						.build(),
				new NamedRoute.Builder().method(Method.GET)
						.path("/sample/{id}")
						.uniqueName("template_extension:sample/get")
						.handler(readHandler)
						.build(),
				new NamedRoute.Builder().method(Method.POST)
						.path("/sample/{id}")
						.uniqueName("template_extension:sample/post")
						.handler(updateHandler)
						.build(),
				new NamedRoute.Builder().method(Method.DELETE)
						.path("/sample/{id}")
						.uniqueName("template_extension:sample/delete")
	...
	Function<RestRequest, RestResponse> createHandler = (request) -> {
		...
		try {
			...
			BooleanResponse exists = client.indices().exists(new ExistsRequest.Builder().index("templateSample").build());
			if (!exists.value()) {
				client.indices().create(new CreateIndexRequest.Builder().index("templateSample").build());
			}
			// Now add our document
			CrudData crudData = new CrudData();
			crudData.setValue("value");
			response = client.index(new IndexRequest.Builder<CrudData>().index("templateSample").document(crudData).build());
	...
	Function<RestRequest, RestResponse> readHandler = (request) -> {
		...
		try {
			response = client.get(new GetRequest.Builder().index("templateSample").id(id).build(), CrudData.class);
		} catch (OpenSearchException | IOException e) {
			return exceptionalRequest(request, e);
		}
		if (response.found()) {
	...
	Function<RestRequest, RestResponse> updateHandler = (request) -> {
		...
		try {
			response = client.update(
					new UpdateRequest.Builder<CrudData, CrudData>().index("templateSample").id(id).doc(crudData).build(),
					CrudData.class
			);
		} catch (OpenSearchException | IOException e) {
			return exceptionalRequest(request, e);
	...
	Function<RestRequest, RestResponse> deleteHandler = (request) -> {
		...
		String id = request.param("id");
		try {
			response = client.delete(new DeleteRequest.Builder().index("templateSample").id(id).build());
		} catch (OpenSearchException | IOException e) {
			return exceptionalRequest(request, e);
		}
